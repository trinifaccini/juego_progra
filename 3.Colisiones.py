...
while True:
    ...
    # Para cambiarlas de color cuando estan colisionando 

    #en caso de colisionar, que cambie de color (se deben crear funciones para esto)
    if rectangulo_vertical.colliderect(rectangulo_horizontal): #VERIFICA SI COLICIONA CON OTRO RECTANGULO
        imagen_horizontal.fill(BLANCO)
        imagen_vertical.fill(ROJO)
    else: #si no colisiona, sigue igual
        imagen_horizontal.fill(AZUL)
        imagen_vertical.fill(VERDE)
